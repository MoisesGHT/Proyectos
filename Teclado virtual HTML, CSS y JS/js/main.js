...
function renderKeyboard(){
    const KeyboarContainer = document.querySelector("#keyboard-container");
    let empty = `<div class="key-empty></div>`;

    const layers = keys.map((layer) => {
        return layer.map(key => {
            if(key[0] == 'SHIFT'){
                return `<button class="key-shift">${key[0]}</button>`;
            }
            if(key[0] == 'MAYUS'){
                return `<button class="key-mayus">${key[0]}</button>`;
            }
            if(key[0] == 'SPACE'){
                return `<button class="key-space"></button>`;
            }
    ...
    htmlLayers.forEach(layer => {
        KeyboarContainer.innerHTML += `<div class="layer">${layer}</div>`;
    });
